...
def main(args):
    ...
    while len(query_votes_dict) > 0:
        ...
        if len(queries_by_subtree)> 0:        
            print ("votes before: {}, votes after: {}".format(votes_b4, len(list(lf_votes.elements()))))
            print ("queries left: {}".format(len(query_votes_dict)))
        if len(queries_by_subtree) == 1:
            strike_count += 1
            if strike_count > 5:
                # five strikes allowed
                # at this point hopefully most of the queries have been picked up in a subtree!
                # just give the remaining queries their own tree and break the loop
                for query, top_votes in query_votes_dict.items():
                    node_y = leaf_dict[top_votes[0]]
                    labels = utils.subtree_nodes_with_edge_length(tree, node_y, n)
                    subtree = tree.extract_tree_with(labels)
                    subtree_dict[subtree] = {query}
                break
        if len(queries_by_subtree) == 0:
            most_common_index += 1
        else:
    ...
    while len(query_votes_dict) > 0:
        ...
        for label in labels:
            if label in leaf_queries:
                remove_list = set()
                for query, top_vote in leaf_queries[label]:
                    if top_vote == True:
                        if query not in query_votes_dict:
                            remove_list.add(query, top_vote)
                        queries_by_subtree.add(query)
                leaf_queries[label].difference_update(remove_list)
                #queries_by_subtree.update(remove_list)
    ...
    os.system("./fragment_tree_hamming_new {} {} {} {} {} {} {}".format(tmp_tree, n, q_aln, len(q_dict), tmp_output, idx, nbr_closest))
